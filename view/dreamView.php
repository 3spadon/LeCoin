...
<a href="index.php?action=listDreams"><i id='lcarret' class="fas fa-caret-left"></i>Retour au journal</a>

<div id="billet">
    <div id="contenuBillet">
        <h3>
            <?= htmlspecialchars($post['intitule']) ?>
            <em><i id="iconCalendar" class="fas fa-calendar-day"></i><?= $post['dateAjout'] ?></em>
        </h3>

        <p>
            <?= nl2br(htmlspecialchars($post['extra'])) ?><br>
            <?= nl2br(htmlspecialchars($post['note'])) ?>
        </p>
    </div>
